...
class WalletService {
  ...
  async addFunds(userId, amount, source, metadata = {}) {
    const transactionId = `wallet_add_${userId}_${Date.now()}`;
    
    try {
      logger.info(`Ajout de fonds: ${transactionId}`, { userId, amount, source });

      if (!Financial.validateAmount(amount, 100, 1000000)) {
        throw new Error('Montant invalide pour l\'ajout de fonds');
      }
      ...
      const transaction = {
        ...
        status: 'completed',
        source: source,
        description: metadata.description || `Ajout de fonds via ${source}`,
        reference: `DEP_${Date.now()}_${Math.random().toString(36).substr(2, 9).toUpperCase()}`,
        metadata: metadata,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      };

      // Exécuter la transaction : créer la transaction ET mettre à jour le solde
      const operations = [
        {
          table: this.table,
          action: 'insert',
      ...
      logger.info(`Fonds ajoutés avec succès: ${transactionId}`, {
        userId,
        amount,
        transactionId: results[0].id
      });

      return Response.success(results[0], 'Fonds ajoutés avec succès');

    } catch (err) {
      const handledError = Error.handleServiceError(err, 'WalletService.addFunds', {
        transactionId,
        userId,
        amount,
        source
      ...
      logger.error(`Échec ajout de fonds: ${transactionId}`, {
